...
class Books {
    ...
    getBooks(userParam) {
        ...
        for(var item of this.books) {
            //console.log("item: "+this.book.item);
            i=0;
            ...
            for(var description in item) {
                if( (vectOfParam[i])!==undefined ){
                    ...
                    //if( !(typeof(vectOfParam[i+1])==='undefined')){ // inutile perche se c'e' from_year c'e' sempre anche to_year per come abbiamo definito la UI
                        ...
                        console.log("rating: "+ reviews.getBookRating(item.isbn));
                        //if(vectOfParam[6]<=getBookRating(item.isbn)) {
                          //check[6]= true;
                        //}
                ...
                i++;
                console.log("rating: "+ reviews.getBookRating(item.isbn));
                    // punto al rating se non esiste il from_year e di conseguenza non esiste nemmeno il to_years
              }
              fieldOfItem = "";
          }
        ...
        if(booksFind){
            console.log("enter booksfind: "+booksFind);
            return booksFind;
        }
    }
